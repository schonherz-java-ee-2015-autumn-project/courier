...
public class QuartzManagerImpl implements QuartzManagerLocal, QuartzManagerRemote {
	...
	public void initialize(String cronExpr) {
		...
		job.getJobDataMap().put("method", "getUsersViaWebService");
		//Args are read from web.xml
		Object[] jdArgs = new Object[0];
		job.getJobDataMap().put("args", jdArgs);
		//idő alapján való triggeléshez 
		CronTrigger cronTrigger = new CronTrigger("Quartz");
		try {
			//beállítjuk a trigget
			cronTrigger.setCronExpression(cronExpr);
			//létrejön egy Scheduler ami megkapja az alapértelmezett Schedulert a továbbiakhoz.
	...
	//Itt fog lezárulni a job, amint "végzett" de a mi esetünkben ez most az egész alkalmazás alatt fog menni
	@Override
